...
func (client *TestRedisClient) SScan(key string, cursor uint64, match string, count int64) (members []string, nextCursor uint64, err error) {
	membersPart, err := client.SMembers(key)
	if err != nil {
		return members, cursor, err
	}
	if cursor >= uint64(len(membersPart)) {
		return members, 0, nil
	}
	members = membersPart[cursor:]
	nextCursor = cursor + uint64(len(members))
	if nextCursor > uint64(len(membersPart)) {
		nextCursor = 0
	}
